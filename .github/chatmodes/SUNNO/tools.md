...
This document defines the available tools in the SUNNO chatmode system, including their descriptions, parameters, and examples. All file operations (create, edit, delete, move, copy) must be performed using terminal commands only. Do not use direct file edit tools. Always log terminal commands and results for audit and compliance.
## Audit Findings & Guidance (2025-07-27)

SUNNO tools are designed to use both VS Code APIs and terminal commands, depending on feature and environment. Terminal commands are required for some file operations, but VS Code tools (API-based) are preferred for search, code analysis, and diagnostics. Do not install new packages unless explicitly requested by the user. If a required feature cannot be performed with available VS Code tools, prompt the user for approval before proceeding with package installation or external commands.

**Best Practice:**
- Use VS Code tools for listing files, searching, code usages, and diagnostics.
- Use terminal commands for file creation, editing, moving, or deletion if VS Code tools are insufficient.
...
- Always log actions and results for audit and compliance.
...
### 2. codebase
- **Description**: Summarizes the workspace structure, including folders, files, and high-level dependencies. Uses VS Code APIs (list_dir, file_search) for best results. Provides context for planning edits.
- **Parameters**:
...
### 3. changes
- **Description**: File changes (create, edit, move, delete) should use VS Code tools if available. If not, use terminal commands (e.g., `echo`, `mv`, `rm`).
- **Example**:
  - Invocation: Use VS Code file edit tools for supported operations. If blocked, run_in_terminal(command="echo 'Add security note' >> guidelines.md", explanation="Append security note to guidelines.md", isBackground=false).
  - Expected Output: "File updated using VS Code tool or terminal command."

### 4. editFiles
- **Description**: Use VS Code file edit tools for supported file edits. If not possible, use terminal commands (e.g., `sed`, `awk`, `echo`).
- **Example**:
  ...
  - Expected Output: "File edited using VS Code tool or terminal command."

### 5. runTests
- **Description**: Executes tests using VS Code's built-in test runner if available. If not, prompt the user before installing or running external test frameworks (e.g., pytest). Returns pass/fail results and logs.
- **Parameters**:
...
## Patch & Edit Protocol
- When updating SUNNO chatmode markdown files, use the less strict edit tool (insert_edit_into_file) for documentation and logic changes. This tool does not require exact context matching and is preferred for markdown updates.
